...
class TestIAMRoles:
    ...
    def test_lambda_role_exists(self, lamdbarole):
        assert lamdbarole["Role"]["RoleName"] == f"{PROJECT}-lambda-role-{ENV}"

    def test_glue_role_exists(self, gluerole):
        assert gluerole["Role"]["RoleName"] == f"{PROJECT}-glue-role-{ENV}"

    def test_lambda_role_has_correct_service(self, lamdbarole):
        policy = lamdbarole["Role"]["AssumeRolePolicyDocument"]
        services = [s["Principal"]["Service"] for s in policy["Statement"]]
...
class TestGlueResources:
    ...
    def test_glue_database_exists(self):
        response = glue.get_database(Name=f"{PROJECT}_{ENV}")
        assert response["Database"]["Name"] == f"{PROJECT}_{ENV}"

    def test_bronze_to_silver_job_exists(self, gluejob_bronze_silver):
        assert (
            gluejob_bronze_silver["Job"]["Name"] == f"{PROJECT}-bronze-to-silver-{ENV}"
        )

    def test_silver_to_gold_job_exists(self, gluejob_silver_gold):
        assert gluejob_silver_gold["Job"]["Name"] == f"{PROJECT}-silver-to-gold-{ENV}"

    def test_bronze_crawler_exists(self):
        response = glue.get_crawler(Name=f"{PROJECT}-bronze-crawler-{ENV}")
        assert response["Crawler"]["Name"] == f"{PROJECT}-bronze-crawler-{ENV}"

    def test_silver_crawler_exists(self):
        response = glue.get_crawler(Name=f"{PROJECT}-silver-crawler-{ENV}")
        assert response["Crawler"]["Name"] == f"{PROJECT}-silver-crawler-{ENV}"

    def test_gold_crawler_exists(self):
        response = glue.get_crawler(Name=f"{PROJECT}-gold-crawler-{ENV}")
        assert response["Crawler"]["Name"] == f"{PROJECT}-gold-crawler-{ENV}"
